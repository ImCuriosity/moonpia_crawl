...
대량 수집 386건 중 385건 성공 (실패 1건 = 비공개·계약종료)
조인 무결성 100% (고아 레코드 0건, 중복 0건)

모델 성능 (GroupKFold 교차검증, 기저선 0.500)
  회차 이탈   ROC-AUC 0.896 ± 0.007   (작품 298 · 회차 5,918)
  독자 이탈   ROC-AUC 0.851 ± 0.009   (독자 2,297 · 등장 6,161)
  작품 간     ROC-AUC 0.849 ± 0.059   (작품 260)
```
...
### 회차 이탈 — 작품 298개 · 회차 5,918건 (CV AUC 0.896)

```
 순위          요인          컬럼수     중요도    표준편차  0과구분
  1    직전 회차 성과          8   0.0666   0.0080     O
  2       연재 위치           1   0.0280   0.0049     O
  3    독자 반응 구성         12   0.0200   0.0065     O
  4  작품 인기도(선작)         1   0.0116   0.0041     O
  5   연재 리듬(공백)          2   0.0067   0.0029     O
  6      페이월 구조          5   0.0056   0.0031     -
  7          장르           1   0.0009   0.0007     -
  8    작품 규모·상태         2   0.0002   0.0005     -
  9 댓글 감정(KOTE)         19  -0.0001   0.0003     -
 10       회차 분량          1  -0.0003   0.0004     -
```

**직전 회차의 성과가 압도적입니다.** 2위 연재 위치(초반일수록 위험)의 2.4배이고,
나머지를 다 합친 것보다 큽니다. 이탈은 갑자기 오지 않고 **직전 회차에서 이미
징후가 보인다**는 뜻입니다.

### 독자 이탈 — 독자 2,297명 · 등장 6,161건 (CV AUC 0.851)

```
 순위       요인      컬럼수     중요도    표준편차  0과구분
  1 독자 참여 이력      4   0.1935   0.0192     O
  2    연재 위치       2   0.0703   0.0098     O
  3 댓글 작성 양상      8   0.0206   0.0080     O
  4 독자 이탈 징후      1   0.0049   0.0042     -
```

독자가 떠날지는 **그 독자가 지금까지 얼마나 참여했는가**로 거의 결정됩니다.
많이 쓴 독자, 연속으로 따라온 독자는 안 떠납니다.

### 작품 간 비교 — 작품 260개 (CV AUC 0.849)

```
 순위          요인       컬럼수     중요도    표준편차  0과구분
  1    독자 반응 구성       6   0.0658   0.0441     -
  2  작품 인기도(선작)      1   0.0601   0.0325     -
  3     독자 인구통계       6   0.0194   0.0279     -
  ...  (이하 전부 0 부근)
```

**여기서는 아직 아무것도 유의하지 않습니다.** 모델은 AUC 0.849로 잘 맞히는데
(±0.059), 어느 요인 때문인지 특정할 수 없습니다. 작품 260개에 피처 33개라
...
### ⚠️ 댓글 감정이 0으로 나온 것은 "중요하지 않다"는 뜻이 아닙니다

전체 데이터에서 감정 커버리지는 **3.3%** 입니다. 댓글은 무료 회차에만 있고,
대량 수집은 속도를 위해 댓글을 뺐기 때문입니다. 96%가 결측인 피처의 순열
중요도는 "기여가 없다"가 아니라 **"잴 표본이 없다"** 입니다.

그래서 댓글이 있는 작품만(커버리지 87.7%) 따로 재봤습니다:

```
작품 10개 · 회차 219행 · 감정 커버리지 87.7%   (CV AUC 0.857)
 순위          요인       중요도    표준편차
  1    직전 회차 성과    0.0815   0.0780
  ...
 10 댓글 감정(KOTE)   -0.0107   0.0289
```

**여전히 기여가 확인되지 않습니다.** 커버리지를 올려도 그렇습니다. 표본이 219행뿐이라
단정할 수는 없지만, 지금 데이터로는 감정이 이탈 예측에 **추가로** 기여한다고 말할 수
없습니다.

단, 단순 상관에서는 부호가 전부 일관됐습니다(`enjoyment` -0.178 ~ `confusion` +0.149).
신호가 없는 게 아니라, **1위 요인인 '직전 회차 성과'가 이미 같은 정보를 담고 있을
가능성**이 있습니다 — 거기에 `prev_returning_commenter_ratio`,
`prev_comment_per_view` 같은 댓글 기반 지표가 들어 있습니다. 확인하려면 유료 구간
댓글이 필요하고, 그건 회차별 구매가 전제입니다.
